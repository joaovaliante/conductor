...
module "loki" {
  source = "gcs::https://www.googleapis.com/storage/v1/cdt-terraform-module/helm/loki"

  name = "loki"
  namespace = "loki"

  values_contents = [
    templatefile("../../../helm/application/loki/values-loki.yaml", {
      registry_password = var.registry_password
    })
  ]
}
...
module "cielopay_cielo" {
  source = "gcs::https://www.googleapis.com/storage/v1/cdt-terraform-module/helm/cielopay"

  name      = "cielopay-cielo"
  namespace = "cielopay-cielo"

  values_contents = [
    templatefile("../../../helm/application/cielopay/values-cielopay-cielo.yaml", {
      registry_password = var.registry_password
    }),
    file("../../../helm/azure/cielopay/values-cielopay.yaml")
  ]
}
...
#   name      = "cielopay-api"
#   namespace = "cielopay-api"

#   values_contents = [
#     templatefile("../../../helm/application/cielopay/values-cielopay-api.yaml", {
#       registry_password = var.registry_password
#     }),
#     file("../../../helm/azure/cielopay/values-cielopay.yaml")
#   ]
...
module "cielopay_bradesco" {
  source = "gcs::https://www.googleapis.com/storage/v1/cdt-terraform-module/helm/cielopay"

  name      = "cielopay-bradesco"
  namespace = "cielopay-bradesco"

  values_contents = [
    templatefile("../../../helm/application/cielopay/values-cielopay-bradesco.yaml", {
      registry_password = var.registry_password
    }),
    file("../../../helm/azure/cielopay/values-cielopay.yaml")
  ]
...
module "rancher_cielopay" {
  ...
  cluster_name  = var.cluster_name
  project       = "Cielopay"
  namespaces    = ["cielopay-cielo", "cielopay-bradesco", "cielopay-cielo-redis", "cielopay-bradesco-redis"]
